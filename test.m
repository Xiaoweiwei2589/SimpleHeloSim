...
%or turn rate
VXTRIM= 1.5;%input('Enter trimmed forward speed in ft/sec:');
...
%for iaz=1:constants.NAZTRIM
 %x0trim(constants.IDXAZ)=(1-1)*daztrim;
 [xdot0,y0]=feval(acfunction,x0trim,u0trim,xdot_in,constants);


 % Here we use the 6-DOF EOM to calculate the initial State Derivatives -- xdot0
 % Also, the initial Output, [Vtot, X/m, Y/m, Z/m]
 % How to do the next step? How to continue this process?


 %xdot0=xdotaz;
 %xdot0+xdotaz/constants.NAZTRIM;
 %y0=yaz;         %y0+yaz/constants.NAZTRIM;
 %end
